...
export default function SchedulerComponent() {
    ...
    const [selectedCalendars, setSelectedCalendars] = useState(["birthday", "holiday", "event"]);
    ...
    const view = "month";
    const [eventData, setEventData] = useState(data); // initial full dataset
    ...
    const addNew = () => {
        ...
        element.whenRendered(() => {
            if (typeof element.openWindow !== "function") {
                console.error("openWindow not available yet");
                return;
            }
    ...
    const handleCalendarChange = (e) => {
        if (!ready || !scheduler.current) return;
        scheduler.current.dateCurrent = e.detail.value;
    };

    // ---------------------------------------------------
    // Fix for Tree Event Filter
    // ---------------------------------------------------
    const handleTreeChange = (e) => {
        const treeEl = tree.current.nativeElement;

        const selected = treeEl.getSelectedValues(); // Smart UI helper

        setSelectedCalendars([...selected]);

        const filtered = data.filter((ev) => selected.includes(ev.class));
    ...
    // ---------------------------------------------------
    // Date change inside Scheduler syncs Calendar
    // ---------------------------------------------------
    const handleDateChange = (e) => {
        if (!ready || !calendar.current) return;
        calendar.current.selectedDates = [e.detail.value];
    };

    // Scheduler Ready Hook → enables everything
    const handleSchedulerReady = () => {
        setReady(true);

        const sched = scheduler.current.nativeElement ?? scheduler.current;
        ...
        <div id="primaryContainer" ref={primaryContainer} className="mobile-collapsed">
            ...
            <div className="content">
                ...
                <section id="sideA" onClick={(e) => e.stopPropagation()} >
                    ...
                    <Button id="drawerCloseBtn" onClick={() => primaryContainer.current.classList.remove("collapse")}>
                        ×
                    </Button>
